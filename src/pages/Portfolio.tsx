      ...
      <div className="flex gap-6 animate-marquee group-hover:[animation-play-state:paused]" style={{ animationDuration: '5s' }}>
        {allIndustries.map((industry, i) => (
          <div
            ...
            className="flex-shrink-0 w-64 p-8 rounded-[4rem] bg-primary/10 text-center hover:bg-primary/15 transition-all duration-300"
          >
        ...
        <section className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 pb-20">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div
                key={project.title}
                className={`group rounded-[4rem] bg-card border border-border hover:border-accent/50 overflow-hidden transition-all duration-300 hover:scale-[1.02] animate-slide-up stagger-${(index % 6) + 1}`}
              >
